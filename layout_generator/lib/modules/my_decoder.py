#!/usr/bin/env python
# codes modified from
#   https://github.com/uvavision/Text2Scene/blob/master/lib/modules/layout_decoder.py
...
class WhatDecoder(nn.Module):
    ...
    def forward(self, prev_states, encoder_states):
        ...
        """
        # summary(self.decoder, (10, 2387))
        # sys.exit()
        #################################################################
        # Conv RNN
        #################################################################
        if len(prev_states) == 2:
            prev_bgfs, prev_fgfs = prev_states
            prev_hids = None
        else:
...
class WhereDecoder(nn.Module):
    ...
    def forward(self, what_states, encoder_states):
        ...
        if self.cfg.what_attn: #True
            if self.cfg.where_attn == 0: #2
                att_ctx = what_ctx
            else:
                encoder_feats = enc_rfts
                if self.cfg.attn_emb:
        ...
        # summary(self.decoder, (1619, 28, 28))
        logits = self.decoder(combined)
        ...
        # print(scale_logits.shape)
        # print(ratio_logits.shape)
        # sys.exit()
...
class MyWhereDecoder(nn.Module):
    def __init__(self, config):
        super(MyWhereDecoder, self).__init__()
        ...
        if config.use_bn:
            ...
            self.decoder = nn.Sequential(
                ...
                nn.Conv2d(tgt_dim, tgt_dim//2, kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(),
                nn.Conv2d(tgt_dim//2, tgt_dim//2//2, kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(),
                nn.Conv2d(tgt_dim//2//2, output_dim, kernel_size=3, stride=1, padding=1),
                nn.LeakyReLU(),
                Flatten(),
                nn.Linear(output_dim*28*28, 4),
                nn.Sigmoid(),
            )
    ...
    def forward(self, what_states, encoder_states):
        ...
        combined = combined.view(bsize * tlen, fsize, gh, gw)
        # print('combined',combined.shape)
        # summary(self.decoder, (2387, 28, 28))
        pred_boxes = self.decoder(combined)
        # print('logits',logits.shape)
        nsize, _ = pred_boxes.size()
        assert(nsize == bsize * tlen)
        pred_boxes = pred_boxes.view(bsize, tlen, -1) 
        # print('logits ', logits.size())

        return pred_boxes, att_ctx, att_wei
